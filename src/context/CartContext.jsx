...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchCart = async () => {
      setLoaded(false);
      const resp = await fetch(`https://ecc.vnarnav.online/api/cart?cartId=${cartId}`);
  ...
  const modifyCart = async (prodId, qty, action = "add") => {
    ...
    try {
      const resp = await fetch("https://ecc.vnarnav.online/api/cart", {
        method: "POST",
  ...
  const deleteItem = async (id) => {
    ...
    try {
      const resp = await fetch(`https://ecc.vnarnav.online/api/cart/${id}?cartId=${cartId}`, {
        method: "DELETE",
      });
      const data = await resp.json();
      setCart(data.cart);
  ...
  const checkout = async (name, email) => {
    ...
    try {
      const resp = await fetch("https://ecc.vnarnav.online/api/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
